...
export default class managers extends Component {
    ...
    componentDidMount() {
       ...
       axios.get('http://localhost:9090/bookapi/api/user/managers').then((response)=>{
        this.setState({
    ...
    render() {
        ...
        let managers = this.state.managers.map((p) => {
            ...
            <td>{p.role}</td>
                ...
                : 
                <CButton color="warning" size="sm" className="btn-pill btn btn-warning btn-sm" >Active</CButton>
                }</td>
                <th>
                    <center>
                        {
